...
- (void)viewDidLoad
{
    ...
    NSString *carrierID = (NSString *)[(id<SODataProperty>)self.carrier.properties[kAirlineID] value];
    NSString *carrierFlightsURL = [NSString stringWithFormat:@"%@('%@')/%@", kCarrierCollection, carrierID, kCarrierFlights];
...
{
    SODataRequestParamSingleDefault *requestParam = [[SODataRequestParamSingleDefault alloc] initWithMode:SODataRequestModeRead resourcePath:query];
    [self.appDelegate.flightsStore scheduleRequest:requestParam delegate:self];
}
